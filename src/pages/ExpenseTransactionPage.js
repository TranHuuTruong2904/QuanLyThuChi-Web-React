...
const ExpenseTransactionPage = () => {
  ...
  const [listTransactionExpense, setListTransactionExpense] = useState([]);
  ...
  async function getTotalDay() {
    const result = await axiosApiInstance.get(
      axiosApiInstance.defaults.baseURL + `/api/transaction/gettotalexpensetoday`
    );
  ...
  async function getTotalWeek() {
    const result = await axiosApiInstance.get(
      axiosApiInstance.defaults.baseURL + `/api/transaction/totalexpensebyweek`
    );
    if (result?.data?.status === 101) {
      setTotalWeek(0);
    } else {
  ...
  async function getListTransactionExpense() {
    const result = await axiosApiInstance.get(
      axiosApiInstance.defaults.baseURL + `/api/transaction/expense`
    );
    setLoad(true);
    setListTransactionExpense(result?.data?.data);
  ...
  const addTransaction = async (idCategory,idCard) => {
    try {
      const result = await axiosApiInstance.post(
        axiosApiInstance.defaults.baseURL +
          `/api/transaction/add/expense/${idCategory}/${idCard}`,
        newTransaction
      );
      if (result?.data?.message.includes("Số tiền trong thẻ không đủ để thực hiện giao dịch")) {
          toast.error(result?.data?.message);
      }
      else{
        setListTransactionExpense([...listTransactionExpense, result?.data?.data]);
        setListTempTransaction([...listTransactionExpense, result?.data?.data]);
  ...
  const updateTransaction = async (idCategory, idTransaction) => {
    try {
      ...
      const result = await axiosApiInstance.put(
        axiosApiInstance.defaults.baseURL +
          `/api/transaction/update/${idCategory}/${idTransaction}`,
        payload
      );
  ...
  async function deleteTransaction(id){
    try {
        ...
        if(result?.data?.status === 200)
        {
          ...
          const newListTransaction = listTransactionExpense.filter((transaction) => transaction.id !== id);
          setListTransactionExpense(newListTransaction);
  ...
  const handleSearch = (event) => {
    ...
    if (searchText === "") {
      setListTempTransaction(listTransactionExpense);
      setCurrentPage(1);
    } else {
    ...
    <>
      ...
      {load ? (
        <div className="container-fluid mx-auto" style={{ width: "95%" }}>
          ...
          >
            <h4>Danh sách giao dịch chi tiêu</h4>
          </div>
